using System.IO;
using System.Runtime.Serialization;
using WolvenKit.CR2W.Reflection;
using static WolvenKit.CR2W.Types.Enums;


namespace WolvenKit.CR2W.Types
{
	[DataContract(Namespace = "")]
	[REDMeta]
	public class CBTTaskSendInfo : IBehTreeTask
	{
		[Ordinal(0)] [RED("("onIsAvailable")] 		public CBool OnIsAvailable { get; set;}

		[Ordinal(0)] [RED("("onActivate")] 		public CBool OnActivate { get; set;}

		[Ordinal(0)] [RED("("onDectivate")] 		public CBool OnDectivate { get; set;}

		[Ordinal(0)] [RED("("infoType")] 		public CEnum<EActionInfoType> InfoType { get; set;}

		[Ordinal(0)] [RED("("useCombatTarget")] 		public CBool UseCombatTarget { get; set;}

		[Ordinal(0)] [RED("("distanceToBecomeUnawareOfOldTarget")] 		public CFloat DistanceToBecomeUnawareOfOldTarget { get; set;}

		[Ordinal(0)] [RED("("lastTarget")] 		public CHandle<CNode> LastTarget { get; set;}

		public CBTTaskSendInfo(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name){ }

		public static new CVariable Create(CR2WFile cr2w, CVariable parent, string name) => new CBTTaskSendInfo(cr2w, parent, name);

		public override void Read(BinaryReader file, uint size) => base.Read(file, size);

		public override void Write(BinaryWriter file) => base.Write(file);

	}
}