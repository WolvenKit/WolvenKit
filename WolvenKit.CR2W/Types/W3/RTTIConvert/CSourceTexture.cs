using System.IO;
using System.Runtime.Serialization;
using WolvenKit.CR2W.Reflection;
using static WolvenKit.CR2W.Types.Enums;


namespace WolvenKit.CR2W.Types
{
	[DataContract(Namespace = "")]
	[REDMeta]
	public class CSourceTexture : CResource
	{
		[Ordinal(0)] [RED("width")] 		public CUInt32 Width { get; set;}

		[Ordinal(0)] [RED("height")] 		public CUInt32 Height { get; set;}

		[Ordinal(0)] [RED("pitch")] 		public CUInt32 Pitch { get; set;}

		[Ordinal(0)] [RED("format")] 		public CEnum<ETextureRawFormat> Format { get; set;}

		public CSourceTexture(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name){ }

		public static new CVariable Create(CR2WFile cr2w, CVariable parent, string name) => new CSourceTexture(cr2w, parent, name);

		public override void Read(BinaryReader file, uint size) => base.Read(file, size);

		public override void Write(BinaryWriter file) => base.Write(file);

	}
}