using System.IO;using System.Runtime.Serialization;
using WolvenKit.CR2W.Reflection;
using static WolvenKit.CR2W.Types.Enums;


namespace WolvenKit.CR2W.Types
{
	[DataContract(Namespace = "")]
	[REDMeta]
	public class CActionMoveAnimationProxy : CObject
	{
		[RED("isInitialized")] 		public CBool IsInitialized { get; set;}

		[RED("isValid")] 		public CBool IsValid { get; set;}

		[RED("duration")] 		public CFloat Duration { get; set;}

		[RED("prevTime")] 		public CFloat PrevTime { get; set;}

		[RED("currTime")] 		public CFloat CurrTime { get; set;}

		[RED("finished")] 		public CBool Finished { get; set;}

		public CActionMoveAnimationProxy(CR2WFile cr2w) : base(cr2w){ }

		public override void Read(BinaryReader file, uint size) => base.Read(file, size);

		public override void Write(BinaryWriter file) => base.Write(file);

		public override CVariable Create(CR2WFile cr2w) => new CActionMoveAnimationProxy(cr2w);

	}
}