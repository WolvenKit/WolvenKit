namespace WolvenKitUI.ViewModels
{
    using Catel.IoC;
    using Catel.MVVM;
    using global::Settings.Interfaces;
	...
	/// </summary>
	internal class ThemeViewModel : ViewModelBase
	{
		...
		public void ApplyTheme(FrameworkElement fe, string themeName)
		{
			if (themeName != null)
			{
				...
				try
				{
					...
					ServiceLocator.Default.ResolveType<IAppearanceManager>().SetTheme(settings.Themes, themeName, AccentColor);
		...
		internal void InitThemes(ISettingsManager settings)
		{
			...
			if (themes != null)
			{
				...
				var defaultTheme = ServiceLocator.Default.ResolveType<IAppearanceManager>().GetDefaultTheme().DisplayName;
