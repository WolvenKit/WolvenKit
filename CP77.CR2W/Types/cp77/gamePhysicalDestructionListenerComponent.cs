using System.IO;
using WolvenKit.CR2W.Reflection;
using FastMember;
using static WolvenKit.CR2W.Types.Enums;

namespace WolvenKit.CR2W.Types
{
	[REDMeta]
	public class gamePhysicalDestructionListenerComponent : entIComponent
	{
		[Ordinal(0)]  [RED("physicalDestructionComponentName")] public CName PhysicalDestructionComponentName { get; set; }
		[Ordinal(1)]  [RED("thresholdLevels")] public CArray<CFloat> ThresholdLevels { get; set; }

		public gamePhysicalDestructionListenerComponent(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
