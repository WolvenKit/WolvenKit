using System.IO;
using WolvenKit.CR2W.Reflection;
using FastMember;
using static WolvenKit.CR2W.Types.Enums;

namespace WolvenKit.CR2W.Types
{
	[REDMeta]
	public class physicsCollisionFilterResource : ISerializable
	{
		[Ordinal(0)]  [RED("collisionGroups")] public CStatic<64,CName> CollisionGroups { get; set; }
		[Ordinal(1)]  [RED("overridesJson")] public rRef<JsonResource> OverridesJson { get; set; }
		[Ordinal(2)]  [RED("presetJson")] public rRef<JsonResource> PresetJson { get; set; }
		[Ordinal(3)]  [RED("queryGroups")] public CStatic<64,CName> QueryGroups { get; set; }

		public physicsCollisionFilterResource(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
