using System.IO;
using CP77.CR2W.Reflection;
using FastMember;
using static CP77.CR2W.Types.Enums;

namespace CP77.CR2W.Types
{
	[REDMeta]
	public class animDangleConstraint_SimulationDyng : animDangleConstraint_Simulation
	{
		[Ordinal(0)]  [RED("HACK_checkDangleTeleport")] public CBool HACK_checkDangleTeleport { get; set; }
        

		[Ordinal(2)]  [RED("substepTime")] public CFloat SubstepTime { get; set; }
		[Ordinal(3)]  [RED("solverIterations")] public CUInt32 SolverIterations { get; set; }
		[Ordinal(4)]  [RED("particlesContainer")] public animDyngParticlesContainer ParticlesContainer { get; set; }
		[Ordinal(5)]  [RED("dyngConstraint")] public CHandle<animIDyngConstraint> DyngConstraint { get; set; }


        [Ordinal(999)] [RED("drawDebugConstraint")] public CBool drawDebugConstraint { get; set; }

		public animDangleConstraint_SimulationDyng(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
