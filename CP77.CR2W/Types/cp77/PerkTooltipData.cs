using System.IO;
using WolvenKit.CR2W.Reflection;
using FastMember;
using static WolvenKit.CR2W.Types.Enums;

namespace WolvenKit.CR2W.Types
{
	[REDMeta]
	public class PerkTooltipData : BasePerksMenuTooltipData
	{
		[Ordinal(0)]  [RED("attributeData")] public CHandle<AttributeData> AttributeData { get; set; }
		[Ordinal(1)]  [RED("attributeId")] public TweakDBID AttributeId { get; set; }
		[Ordinal(2)]  [RED("perkArea")] public CEnum<gamedataPerkArea> PerkArea { get; set; }
		[Ordinal(3)]  [RED("perkData")] public CHandle<PerkDisplayData> PerkData { get; set; }
		[Ordinal(4)]  [RED("perkType")] public CEnum<gamedataPerkType> PerkType { get; set; }
		[Ordinal(5)]  [RED("proficiency")] public CEnum<gamedataProficiencyType> Proficiency { get; set; }

		public PerkTooltipData(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
