using System.IO;
using WolvenKit.CR2W.Reflection;
using FastMember;
using static WolvenKit.CR2W.Types.Enums;

namespace WolvenKit.CR2W.Types
{
	[REDMeta]
	public class meshMeshParamShadowMeshCreationData : meshMeshParameter
	{
		[Ordinal(0)]  [RED("bonesPerGeometry")] public CArray<CInt32> BonesPerGeometry { get; set; }
		[Ordinal(1)]  [RED("geometries")] public CArray<CHandle<physicsICollider>> Geometries { get; set; }

		public meshMeshParamShadowMeshCreationData(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
