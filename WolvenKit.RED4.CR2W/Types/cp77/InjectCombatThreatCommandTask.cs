...
{
	[REDMeta]
	public class InjectCombatThreatCommandTask : AIbehaviortaskScript
	{
		private CHandle<AIArgumentMapping> _inCommand;
		private wCHandle<AIInjectCombatThreatCommand> _currentCommand;
		...
		[Ordinal(1)] 
		[RED("currentCommand")] 
		public wCHandle<AIInjectCombatThreatCommand> CurrentCommand
		{
			get
			{
				if (_currentCommand == null)
				{
					_currentCommand = (wCHandle<AIInjectCombatThreatCommand>) CR2WTypeManager.Create("whandle:AIInjectCombatThreatCommand", "currentCommand", cr2w, this);
				}
