	...
	TextureCreationFlags(0), OverrideMaterial2DEnabled(false), AllowZWriteOnTransparent(false)
{
	...
	DriverAttributes = DBG_NEW io::CAttributes();
...
//	DriverAttributes->addInt("AntiAlias", 0);
	...
	// create manipulator
	MeshManipulator = DBG_NEW scene::CMeshManipulator();
    ...
    SurfaceLoader.push_back(video::createImageLoaderXBM());
#endif
...
ITexture* CNullDriver::addTexture(const core::dimension2d<u32>& size, const io::path& name, ECOLOR_FORMAT format)
{
	...
	IImage* image = DBG_NEW CImage(format, size);
	ITexture* t = 0;
...
ITexture* CNullDriver::addTextureCubemap(const irr::u32 sideLen, const io::path& name, ECOLOR_FORMAT format)
{
	...
	core::array<IImage*> imageArray(6);
	for ( int i=0; i < 6; ++i )
		imageArray.push_back(DBG_NEW CImage(format, core::dimension2du(sideLen, sideLen)));
...
void CNullDriver::makeNormalMapTexture(video::ITexture* texture, f32 amplitude) const
{
	...
	if (texture->getColorFormat() == ECF_A8R8G8B8)
	{
		...
		s32* in = DBG_NEW s32[dim.Height * pitch];
	...
	else
	{
		...
		s16* in = DBG_NEW s16[dim.Height * pitch];
...
IImage* CNullDriver::createImageFromData(ECOLOR_FORMAT format,
	...
	bool deleteMemory)
{
	return DBG_NEW CImage(format, size, data, ownForeignMemory, deleteMemory);
}
...
{
	return DBG_NEW CImage(format, size);
}
...
IImage* CNullDriver::createImage(ECOLOR_FORMAT format, IImage *imageToCopy)
{
	...
	CImage* tmp = DBG_NEW CImage(format, imageToCopy->getDimension());
...
{
	os::Printer::log("Deprecated method, please create an empty image instead and use copyTo().", ELL_WARNING);
	CImage* tmp = DBG_NEW CImage(imageToCopy->getColorFormat(), imageToCopy->getDimension());
...
IImage* CNullDriver::createImage(ITexture* texture, const core::position2d<s32>& pos, const core::dimension2d<u32>& size)
{
	if ((pos==core::position2di(0,0)) && (size == texture->getSize()))
	{
		...
		if ( !data)
			return 0;
		IImage* image = DBG_NEW CImage(texture->getColorFormat(), size, data, false, false);
		texture->unlock();
		return image;
	}
	else
	{
		...
		if (!src)
			return 0;
		IImage* image = DBG_NEW CImage(texture->getColorFormat(), clamped.getSize());
...
{
	io::CAttributes* attr = DBG_NEW io::CAttributes(this);
...
{
	CNullDriver* nullDriver = DBG_NEW CNullDriver(io, screenSize);

	// create empty material renderers
	for(u32 i=0; sBuiltInMaterialTypeNames[i]; ++i)
	{
		IMaterialRenderer* imr = DBG_NEW IMaterialRenderer();
