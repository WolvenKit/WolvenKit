...
void CD3D9Driver::createMaterialRenderers()
{
	...
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_SOLID(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_SOLID_2_LAYER(pID3DDevice, this));

	// add the same renderer for all lightmap types

	CD3D9MaterialRenderer_LIGHTMAP* lmr = DBG_NEW CD3D9MaterialRenderer_LIGHTMAP(pID3DDevice, this);
	...
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_DETAIL_MAP(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_SPHERE_MAP(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_REFLECTION_2_LAYER(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_TRANSPARENT_ADD_COLOR(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_TRANSPARENT_ALPHA_CHANNEL(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_TRANSPARENT_ALPHA_CHANNEL_REF(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_TRANSPARENT_VERTEX_ALPHA(pID3DDevice, this));
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_TRANSPARENT_REFLECTION_2_LAYER(pID3DDevice, this));
	...
	renderer = DBG_NEW CD3D9NormalMapRenderer(pID3DDevice, this, tmp,
	...
	renderer = DBG_NEW CD3D9ParallaxMapRenderer(pID3DDevice, this, tmp,
		MaterialRenderers[EMT_SOLID].Renderer);
	renderer->drop();
	...
	addAndDropMaterialRenderer(DBG_NEW CD3D9MaterialRenderer_ONETEXTURE_BLEND(pID3DDevice, this));
}
...
bool CD3D9Driver::initDriver(HWND hwnd, bool pureSoftware)
{
    ...
    if (!BridgeCalls)
		BridgeCalls = DBG_NEW CD3D9CallBridge(pID3DDevice, this);
...
ITexture* CD3D9Driver::createDeviceDependentTexture(const io::path& name, IImage* image)
{
	...
	CD3D9Texture* texture = DBG_NEW CD3D9Texture(name, imageArray, ETT_2D, this);
...
{
	CD3D9Texture* texture = DBG_NEW CD3D9Texture(name, image, ETT_CUBEMAP, this);
...
CD3D9Driver::SHWBufferLink *CD3D9Driver::createHardwareBuffer(const scene::IMeshBuffer* mb)
{
	...
	SHWBufferLink_d3d9 *hwBuffer=DBG_NEW SHWBufferLink_d3d9(mb);
...
{
	CD3D9RenderTarget* renderTarget = DBG_NEW CD3D9RenderTarget(this);
...
{
	s32 nr = -1;
	CD3D9ShaderMaterialRenderer* r = DBG_NEW CD3D9ShaderMaterialRenderer(
		...
		E_MATERIAL_TYPE baseMaterial, s32 userData, E_GPU_SHADING_LANGUAGE shadingLang)
{
	...
	CD3D9HLSLMaterialRenderer* r = DBG_NEW CD3D9HLSLMaterialRenderer(
			pID3DDevice, this, nr,
			vertexShaderProgram,
...
{
	CD3D9Texture* tex = DBG_NEW CD3D9Texture(this, size, name, ETT_2D, format);
	if (tex)
	{
		if (!tex->Texture)
		{
...
{
	CD3D9Texture* tex = DBG_NEW CD3D9Texture(this, core::dimension2d<u32>(sideLen, sideLen), name, ETT_CUBEMAP, format);
	if (tex)
	{
		if (!tex->CubeTexture)
		{
			...
			io::IFileSystem* io, HWND window)
{
	...
	CD3D9Driver* dx9 = DBG_NEW CD3D9Driver(params, io);
