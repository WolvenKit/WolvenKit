...
public:
	...
	video::IVideoDriver* getVideoDriver() const _IRR_OVERRIDE_;

	//! returns pointer to the filesystem
	io::IFileSystem* getFileSystem() const _IRR_OVERRIDE_;

	//! returns a pointer to the OS operator
	IOSOperator* getOSOperator() const _IRR_OVERRIDE_;
	...
	void setUserEventReceiver(IEventReceiver* evr) _IRR_OVERRIDE_;
	...
	IGUISkin* getSkin() const _IRR_OVERRIDE_;

	//! Sets a new GUI Skin
	void setSkin(IGUISkin* skin) _IRR_OVERRIDE_;

	//! Creates a new GUI Skin based on a template.
	/** \return Returns a pointer to the created skin.
	If you no longer need the skin, you should call IGUISkin::drop().
	See IReferenceCounted::drop() for more information. */
	IGUISkin* createSkin(EGUI_SKIN_TYPE type) _IRR_OVERRIDE_;

	//! Creates the image list from the given texture.
	IGUIImageList* createImageList(video::ITexture* texture,
					core::dimension2d<s32> imageSize, bool useAlphaChannel) _IRR_OVERRIDE_;

	//! returns the font
	IGUIFont* getFont(const io::path& filename) _IRR_OVERRIDE_;

	//! add an externally loaded font
	IGUIFont* addFont(const io::path& name, IGUIFont* font) _IRR_OVERRIDE_;

	//! remove loaded font
	void removeFont(IGUIFont* font) _IRR_OVERRIDE_;
	...
	//! returns the sprite bank
	IGUISpriteBank* addEmptySpriteBank(const io::path& name) _IRR_OVERRIDE_;

	//! adds an button. The returned pointer must not be dropped.
	IGUIButton* addButton(const core::rect<s32>& rectangle, IGUIElement* parent=0, s32 id=-1, const wchar_t* text=0,const wchar_t* tooltiptext = 0) _IRR_OVERRIDE_;

	//! adds a window. The returned pointer must not be dropped.
	IGUIWindow* addWindow(const core::rect<s32>& rectangle, bool modal = false,
	...
	IGUIElement* addModalScreen(IGUIElement* parent) _IRR_OVERRIDE_;

	//! Adds a message box.
	IGUIWindow* addMessageBox(const wchar_t* caption, const wchar_t* text=0,
		bool modal = true, s32 flag = EMBF_OK, IGUIElement* parent=0, s32 id=-1, video::ITexture* image=0) _IRR_OVERRIDE_;

	//! adds a scrollbar. The returned pointer must not be dropped.
	IGUIScrollBar* addScrollBar(bool horizontal, const core::rect<s32>& rectangle,
		IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;

	//! Adds an image element.
	IGUIImage* addImage(video::ITexture* image, core::position2d<s32> pos,
	...
	//! adds a checkbox
	IGUICheckBox* addCheckBox(bool checked, const core::rect<s32>& rectangle,
	...
	//! adds a tree view
	IGUITreeView* addTreeView(const core::rect<s32>& rectangle,
	...
	IGUIStaticText* addStaticText(const wchar_t* text, const core::rect<s32>& rectangle,
		bool border=false, bool wordWrap=true, IGUIElement* parent=0, s32 id=-1, bool drawBackground = false) _IRR_OVERRIDE_;

	//! Adds an edit box. The returned pointer must not be dropped.
	IGUIEditBox* addEditBox(const wchar_t* text, const core::rect<s32>& rectangle,
		bool border=false, IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;

	//! Adds a spin box to the environment
	IGUISpinBox* addSpinBox(const wchar_t* text, const core::rect<s32>& rectangle,
		bool border=false,IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;
	...
	IGUIContextMenu* addMenu(IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;

	//! Adds a toolbar to the environment. It is like a menu is always placed on top
	//! in its parent, and contains buttons.
	IGUIToolBar* addToolBar(IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;
	...
	IGUITable* addTable(const core::rect<s32>& rectangle,
		IGUIElement* parent=0, s32 id=-1, bool drawBackground=false) _IRR_OVERRIDE_;

	//! Adds an element to display the information from the Irrlicht profiler
	IGUIProfiler* addProfilerDisplay(const core::rect<s32>& rectangle,
		IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;

	//! sets the focus to an element
	bool setFocus(IGUIElement* element) _IRR_OVERRIDE_;

	//! removes the focus from an element
	bool removeFocus(IGUIElement* element) _IRR_OVERRIDE_;

	//! Returns if the element has focus
	bool hasFocus(const IGUIElement* element, bool checkSubElements=false) const _IRR_OVERRIDE_;

	//! Returns the element with the focus
	IGUIElement* getFocus() const _IRR_OVERRIDE_;

	//! Returns the element last known to be under the mouse
	IGUIElement* getHovered() const _IRR_OVERRIDE_;

	//! Adds an element for fading in or out.
	IGUIInOutFader* addInOutFader(const core::rect<s32>* rectangle=0, IGUIElement* parent=0, s32 id=-1) _IRR_OVERRIDE_;

	//! Returns the root gui element.
	IGUIElement* getRootGUIElement() _IRR_OVERRIDE_;

	void OnPostRender( u32 time ) _IRR_OVERRIDE_;

	//! Returns the default element factory which can create all built in elements
	IGUIElementFactory* getDefaultGUIElementFactory() const _IRR_OVERRIDE_;

	//! Adds an element factory to the gui environment.
	/** Use this to extend the gui environment with new element types which it should be
	able to create automatically, for example when loading data from xml files. */
	void registerGUIElementFactory(IGUIElementFactory* factoryToAdd) _IRR_OVERRIDE_;

	//! Returns amount of registered scene node factories.
	u32 getRegisteredGUIElementFactoryCount() const _IRR_OVERRIDE_;

	//! Returns a scene node factory by index
	IGUIElementFactory* getGUIElementFactory(u32 index) const _IRR_OVERRIDE_;

	//! Adds a GUI Element by its name
	IGUIElement* addGUIElement(const c8* elementName, IGUIElement* parent=0) _IRR_OVERRIDE_;

	//! Saves the current gui into a file.
	/** \param filename: Name of the file.
	\param start: The element to start saving from.
	if not specified, the root element will be used */
	bool saveGUI( const io::path& filename, IGUIElement* start=0) _IRR_OVERRIDE_;

	//! Saves the current gui into a file.
	/** \param file: The file to save the GUI to.
	\param start: The element to start saving from.
	if not specified, the root element will be used */
	bool saveGUI(io::IWriteFile* file, IGUIElement* start=0) _IRR_OVERRIDE_;

	//! Loads the gui. Note that the current gui is not cleared before.
	/** \param filename: Name of the file.
	\param parent: The parent of all loaded GUI elements,
	if not specified, the root element will be used */
	bool loadGUI(const io::path& filename, IGUIElement* parent=0) _IRR_OVERRIDE_;

	//! Loads the gui. Note that the current gui is not cleared before.
	/** \param file: IReadFile to load the GUI from
	\param parent: The parent of all loaded GUI elements,
	if not specified, the root element will be used */
	bool loadGUI(io::IReadFile* file, IGUIElement* parent=0) _IRR_OVERRIDE_;
	...
	//! writes an element
	void writeGUIElement(io::IXMLWriter* writer, IGUIElement* node) _IRR_OVERRIDE_;

	//! reads an element
	void readGUIElement(io::IXMLReader* reader, IGUIElement* node) _IRR_OVERRIDE_;

	//! Find the next element which would be selected when pressing the tab-key
	IGUIElement* getNextElement(bool reverse=false, bool group=false) _IRR_OVERRIDE_;

	//! Set the way the gui will handle focus changes
	void setFocusBehavior(u32 flags) _IRR_OVERRIDE_;

	//! Get the way the gui does handle focus changes
	u32 getFocusBehavior() const _IRR_OVERRIDE_;
