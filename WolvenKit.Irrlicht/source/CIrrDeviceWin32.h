...
namespace irr
{
	...
	public:
		...
		bool run() _IRR_OVERRIDE_;

		//! Cause the device to temporarily pause execution and let other processes to run
		// This should bring down processor usage without major performance loss for Irrlicht
		void yield() _IRR_OVERRIDE_;

		//! Pause execution and let other processes to run for a specified amount of time.
		void sleep(u32 timeMs, bool pauseTimer) _IRR_OVERRIDE_;

		//! sets the caption of the window
		void setWindowCaption(const wchar_t* text) _IRR_OVERRIDE_;

		//! returns if window is active. if not, nothing need to be drawn
		bool isWindowActive() const _IRR_OVERRIDE_;
		...
		bool present(video::IImage* surface, void* windowId=0, core::rect<s32>* src=0) _IRR_OVERRIDE_;

		//! notifies the device that it should close itself
		void closeDevice() _IRR_OVERRIDE_;

		//! \return Returns a pointer to a list with all video modes
		//! supported by the gfx adapter.
		video::IVideoModeList* getVideoModeList() _IRR_OVERRIDE_;
		...
		void setResizable(bool resize=false) _IRR_OVERRIDE_;

		//! Resize the render window.
		void setWindowSize(const irr::core::dimension2d<u32>& size) _IRR_OVERRIDE_;

		//! Minimizes the window.
		void minimizeWindow() _IRR_OVERRIDE_;

		//! Maximizes the window.
		void maximizeWindow() _IRR_OVERRIDE_;

		//! Restores the window size.
		void restoreWindow() _IRR_OVERRIDE_;

		//! Get the position of the window on screen
		core::position2di getWindowPosition() _IRR_OVERRIDE_;

		//! Activate any joysticks, and generate events for them.
		bool activateJoysticks(core::array<SJoystickInfo> & joystickInfo) _IRR_OVERRIDE_;

		//! Set the current Gamma Value for the Display
		bool setGammaRamp( f32 red, f32 green, f32 blue, f32 brightness, f32 contrast ) _IRR_OVERRIDE_;

		//! Get the current Gamma Value for the Display
		bool getGammaRamp( f32 &red, f32 &green, f32 &blue, f32 &brightness, f32 &contrast ) _IRR_OVERRIDE_;

		//! Remove all messages pending in the system message loop
		void clearSystemMessages() _IRR_OVERRIDE_;

		//! Get the device type
		E_DEVICE_TYPE getType() const _IRR_OVERRIDE_
		{
				return EIDT_WIN32;
		}
		...
		u32 checkSuccessiveClicks(s32 mouseX, s32 mouseY, EMOUSE_INPUT_EVENT inputEvent ) _IRR_OVERRIDE_
		{
		...
		public:
			...
			void setVisible(bool visible) _IRR_OVERRIDE_
			{
				CURSORINFO info;
			...
			void setPosition(const core::position2d<f32> &pos) _IRR_OVERRIDE_
			{
				setPosition(pos.X, pos.Y);
			}

			//! Sets the new position of the cursor.
			void setPosition(f32 x, f32 y) _IRR_OVERRIDE_
			{
				if (!UseReferenceRect)
			...
			//! Sets the new position of the cursor.
			void setPosition(const core::position2d<s32> &pos) _IRR_OVERRIDE_
			{
				setPosition(pos.X, pos.Y);
			}

			//! Sets the new position of the cursor.
			void setPosition(s32 x, s32 y) _IRR_OVERRIDE_
			{
				if (UseReferenceRect)
				{
			...
			const core::position2d<s32>& getPosition() _IRR_OVERRIDE_
			{
			...
			//! Returns the current position of the mouse cursor.
			core::position2d<f32> getRelativePosition() _IRR_OVERRIDE_
			{
				updateInternalCursorPosition();
			...
			void setReferenceRect(core::rect<s32>* rect=0) _IRR_OVERRIDE_
			{
				if (rect)
				{
					ReferenceRect = *rect;
			...
			void setActiveIcon(gui::ECURSOR_ICON iconId) _IRR_OVERRIDE_;

			//! Gets the currently active icon
			gui::ECURSOR_ICON getActiveIcon() const _IRR_OVERRIDE_
			{
				return ActiveIcon;
			}

			//! Add a custom sprite as cursor icon.
			gui::ECURSOR_ICON addIcon(const gui::SCursorSprite& icon) _IRR_OVERRIDE_;

			//! replace the given cursor icon.
			void changeIcon(gui::ECURSOR_ICON iconId, const gui::SCursorSprite& icon) _IRR_OVERRIDE_;

			//! Return a system-specific size which is supported for cursors. Larger icons will fail, smaller icons might work.
			core::dimension2di getSupportedIconSize() const _IRR_OVERRIDE_;
