...
: SceneMgr(smgr), FileSystem(filesys)
{
	...
	TextureLoader = DBG_NEW CMeshTextureLoader( FileSystem, SceneMgr->getVideoDriver() );
}
...
IAnimatedMesh* CDMFLoader::createMesh(io::IReadFile* file)
{
	...
	if (GetDMFHeader(dmfRawFile, header))
	{
		...
		dmfVert *verts=DBG_NEW dmfVert[header.numVertices];
		dmfFace *faces=DBG_NEW dmfFace[header.numFaces];
		...
		for (i=0; i<header.numMaterials; i++)
		{
			...
			SSkinMeshBuffer* buffer = DBG_NEW SSkinMeshBuffer();
