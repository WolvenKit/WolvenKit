...
{
	CAnimatedMeshHalfLife* msh = DBG_NEW CAnimatedMeshHalfLife();
	if (msh)
	{
...
void STextureAtlas::create(u32 border, E_TEXTURE_CLAMP texmode)
{
	...
	IImage* master = DBG_NEW CImage(format, dim);
	master->fill(0);
...
SHalflifeHeader* CAnimatedMeshHalfLife::loadModel(io::IReadFile* file, const io::path& filename)
{
	...
	// read into memory
	u8* pin = DBG_NEW u8[file->getSize()];
