	...
	else if (Looping)
	{
		...
		if (FramesPerSecond > 0.f) //forwards...
		{
			if (CurrentFrameNr > EndFrame)
				CurrentFrameNr = StartFrame + (f32)fmod(CurrentFrameNr - StartFrame, (f32)(EndFrame-StartFrame));
		}
		else //backwards...
		{
			if (CurrentFrameNr < StartFrame)
				CurrentFrameNr = EndFrame - (f32)fmod(EndFrame - CurrentFrameNr, (f32)(EndFrame-StartFrame));
		}
	}
	else
	{
...
void CAnimatedMeshSceneNode::updateAbsolutePosition()
{
	...
	if (taglist)
	{
		if (!MD3Special)
		{
			MD3Special = DBG_NEW SMD3Special();
		}
...
ISceneNode* CAnimatedMeshSceneNode::clone(ISceneNode* newParent, ISceneManager* newManager)
{
	...
	CAnimatedMeshSceneNode* newNode =
		DBG_NEW CAnimatedMeshSceneNode(Mesh, NULL, newManager, ID, RelativeTranslation,
