...
IAnimatedMesh* CColladaFileLoader::createMesh(io::IReadFile* file)
{
	...
	if (!DummyMesh)
		DummyMesh = DBG_NEW SAnimatedMesh();
	...
	while(reader->read())
	{
		if (reader->getNodeType() == io::EXN_ELEMENT)
		{
			if (visualSceneSectionName == reader->getNodeName())
				p = DBG_NEW CScenePrefab(readId(reader));
			else
...
void CColladaFileLoader::readNodeSection(io::IXMLReaderUTF8* reader, scene::ISceneNode* parent, CScenePrefab* p)
{
	...
	if (p)
	{
		nodeprefab = DBG_NEW CScenePrefab(readId(reader));
		...
		const core::stringc& type)
{
	...
	if (p)
	{
		...
		{
			Prefabs.push_back(DBG_NEW CGeometryPrefab(url));
...
void CColladaFileLoader::readCameraPrefab(io::IXMLReaderUTF8* reader)
{
	...
	CCameraPrefab* prefab = DBG_NEW CCameraPrefab(readId(reader));
...
void CColladaFileLoader::readEffect(io::IXMLReaderUTF8* reader, SColladaEffect * effect)
{
	...
	if (!effect)
	{
		...
		effect->Parameters = DBG_NEW io::CAttributes();
...
void CColladaFileLoader::readGeometry(io::IXMLReaderUTF8* reader)
{
	...
	scene::SMesh* mesh = DBG_NEW SMesh();
	amesh->addMesh(mesh);
	...
	if (i==Prefabs.size())
	{
		CGeometryPrefab* prefab = DBG_NEW CGeometryPrefab(id);
		prefab->Mesh = mesh;
		Prefabs.push_back(prefab);
	}
		...
		const core::stringc& geometryId)
{
	...
	if ( textureCoordSetCount < 2 )
	{
		...
		scene::SMeshBuffer* mbuffer = DBG_NEW SMeshBuffer();
		buffer = mbuffer;
	...
	else
	{
		...
		scene::SMeshBufferLightMap* mbuffer = DBG_NEW SMeshBufferLightMap();
		buffer = mbuffer;
...
void CColladaFileLoader::readLightPrefab(io::IXMLReaderUTF8* reader)
{
	...
	CLightPrefab* prefab = DBG_NEW CLightPrefab(readId(reader));
		...
		for (u32 i=0; i<Images.size(); ++i)
		{
			if (uri == Images[i].Id)
			{
				...
				if (Images[i].Source.size())
				{
					...
					u32* data = DBG_NEW u32[size]; // we assume RGBA
					u32* ptrdest = data;
