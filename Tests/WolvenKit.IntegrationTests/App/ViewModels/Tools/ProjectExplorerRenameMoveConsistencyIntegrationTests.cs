...
public class ProjectExplorerRenameMoveConsistencyIntegrationTests : IDisposable
{
    ...
    public async Task ImportRenameMoveReimportOverwrite_CollapsesToSingleFile_OnDiskAndInTree()
    {
        ...
        await AssertSingleFile("after first import");
        ...
        await AssertSingleFile("after rename foo->A");

        // 3) rename A -> foo (back)
        await RenameVia(gameA, gameFoo);
        await AssertSingleFile("after rename A->foo");

        // 4) rename foo -> B
        await RenameVia(gameFoo, gameB);
        await AssertSingleFile("after rename foo->B");

        // 5) move B into the parent directory
        await RenameVia(gameB, gameParentB);
        await AssertSingleFile("after move B->parent");
    ...
    private Task AssertSingleFile(string when) => AssertFileCounts(when, 1);

    private async Task AssertFileCounts(string when, int expected)
    {
        // Disk is already consistent when the awaited rename/move returns.
    ...
    private async Task<string> ImportOneFileFromAssetBrowser()
    {
        ...
        Assert.True(
            await AsyncWait.UntilCountSettlesAsync(
                () => _pe!.FileList.Count(m => !m.IsDirectory),
                minimum: 1,
                TimeSpan.FromSeconds(60)),
            "Project explorer never showed the imported file.");
