...
export function validateWorkspotFile(workspot, fixIndexOrder, showUnusedAnimsInFiles, showUndefinedWorkspotAnims, checkIdleAnimNames, checkIdDuplication, checkFilepaths, checkLoadingHandles) {
    ...
    let rootEntry = workspotTree.Data.rootEntry; // can in theory be overwritten as soon as javascript no longer truncates uint64
    ...
    if (false && fixIndexOrder) { // currently deactivated because it breaks uint64
