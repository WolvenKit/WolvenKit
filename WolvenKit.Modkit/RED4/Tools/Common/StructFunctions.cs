...
namespace WolvenKit.Modkit.RED4.GeneralStructs
{
    public class Converters
    {
        public static float hfconvert(ushort read)// for converting ushort representation of a Half float to a float32
        {
            ...
            {
                value = sp == 0 ? float.PositiveInfinity : float.NaN;
            }
            else
            {
    ...
    public class Manipulators
    {
        ...
        {
            var w = ((Q.X * Q.X) + (Q.Y * Q.Y) + (Q.Z * Q.Z)) >= 1f
                ? (float)Math.Sqrt((Q.X * Q.X) + (Q.Y * Q.Y) + (Q.Z * Q.Z) - 1f)
                : (float)Math.Sqrt(1f - ((Q.X * Q.X) + (Q.Y * Q.Y) + (Q.Z * Q.Z)));
