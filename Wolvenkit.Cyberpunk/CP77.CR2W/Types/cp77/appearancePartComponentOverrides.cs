using System.IO;
using CP77.CR2W.Reflection;
using FastMember;
using static CP77.CR2W.Types.Enums;

namespace CP77.CR2W.Types
{
	[REDMeta]
	public class appearancePartComponentOverrides : CVariable
	{
		[Ordinal(0)]  [RED("acceptDismemberment")] public CBool AcceptDismemberment { get; set; }
		[Ordinal(1)]  [RED("chunkMask")] public CUInt64 ChunkMask { get; set; }
		[Ordinal(2)]  [RED("componentName")] public CName ComponentName { get; set; }
		[Ordinal(3)]  [RED("initialTransform")] public Transform InitialTransform { get; set; }
		[Ordinal(4)]  [RED("meshAppearance")] public CName MeshAppearance { get; set; }
		[Ordinal(5)]  [RED("useCustomTransform")] public CBool UseCustomTransform { get; set; }
		[Ordinal(6)]  [RED("visualScale")] public Vector3 VisualScale { get; set; }

		public appearancePartComponentOverrides(CR2WFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
